...
for year in [2017, 2018, 2019, 2020]:
    ...
    year_df = year_df.dropna(subset=['Latitude', 'Longitude', 'Total'])

    # Exclude states with zero crime totals
    year_df = year_df[year_df['Total'] > 0]

    # Features: crime intensity + location
    features = year_df[['Total', 'Latitude', 'Longitude']]
    scaled_features = StandardScaler().fit_transform(features)

    # Spectral Clustering
    spectral = SpectralClustering(n_clusters=2, affinity='nearest_neighbors', assign_labels='kmeans', random_state=0)
    year_df['Cluster'] = spectral.fit_predict(scaled_features)

    # Identify hotspot cluster (highest avg total)
    hotspot_cluster = year_df.groupby('Cluster')['Total'].mean().idxmax()
    year_df['Hotspot_Spectral'] = year_df['Cluster'].apply(lambda x: 1 if x == hotspot_cluster else 0)

    # Create map
    map_year = folium.Map(location=[22.9734, 78.6569], zoom_start=5, tiles="CartoDB positron")

    for _, row in year_df.iterrows():
        color = 'red' if row['Hotspot_Spectral'] == 1 else 'blue'
        folium.CircleMarker(
            ...
            popup=(f"<b>{row['State/UT']}</b><br>"
                   ...
                   f"Spectral Hotspot: {'Yes' if row['Hotspot_Spectral'] == 1 else 'No'}"),
            color=color,
            fill=True,
            fill_color=color,
            fill_opacity=0.7
    ...
    # Save the map
    filename = f"cyber_crime_hotspots_spectral_{year}.html"
    map_year.save(filename)
    print(f"✅ Spectral Clustering hotspot map for {year} saved as '{filename}'")

    hotspot_states = year_df[year_df['Hotspot_Spectral'] == 1][['State/UT', 'Year', 'Total']]
    print(f"\n🔥 Hotspot States Detected by Spectral Clustering for {year}:")
