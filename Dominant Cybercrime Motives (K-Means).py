...
for year in sorted(df_grouped['Year'].unique()):
    ...
    # Save map
    map_file = f"crime_kmeans_clusters_{year}.html"
    m.save(map_file)
    ...
    plt.title(f"State Distribution Across Clusters - {year} (KMeans)")
    ...
    plt.title(f"Dominant Crime Types per Cluster - {year} (KMeans)")
